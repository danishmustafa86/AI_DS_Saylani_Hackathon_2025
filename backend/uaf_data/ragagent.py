...
class UAFKnowledgeBase:
    ...
    def _load_json_data(self):
        ...
        try:
            with open("backend/uaf_scraped_data.json", "r", encoding="utf-8") as f:
                data = json.load(f)
            self.json_docs = [
                Document(
...
class UAFWebSearchTool:
    """Web search tool for UAF University information"""
    
    def __init__(self):
        self.serp_api_key = os.getenv("SERP_API_KEY")
        self.ddgs = DDGS()

    def search_serp_api(self, query: str, max_results: int = 5) -> List[Dict]:
    ...
    def search(self, query: str, max_results: int = 5) -> List[Dict]:
        """Search for UAF University information"""
        uaf_query = f"UAF University Agriculture Faisalabad Pakistan {query}"
        
        # Try SERP API first
        results = self.search_serp_api(uaf_query, max_results)
        
        # Fallback to DuckDuckGo
        if not results:
            results = self.search_duckduckgo(uaf_query, max_results)
...
class UAFAgentTool:
    ...
    def _get_llm_response_fallback(self, messages: List[Dict]) -> str:
        ...
        try:
            api_key = os.getenv("AIMLAPI_KEY")
            if not api_key:
                return "Error: AIMLAPI_KEY not configured"
